...
contract LotteryCore {
  ...
  event PlayeTotalValue(address potPlayer, uint[] playerId, uint playerValue, uint playCount, uint ticketNumber) ;
  ...
  function select_Winner() public ownerOnly {
    ...
    uint winnerIndex = l_randomWords % potPlayersArray.length ;  // potPlayers.length ;
